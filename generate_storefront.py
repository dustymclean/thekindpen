...
CSV_PATH = os.path.expanduser("~/Desktop/KindPen_Scraper/thekindpen_catalog.csv")
OUTPUT_DIR = os.path.expanduser("~/Desktop/KindPen_Shop")
...
def generate_site():
    ...
    for p in products_dict.values():
        if p["in_stock_variants"]:
            p["min_price"] = min(v["price"] for v in p["in_stock_variants"])
            p["featured_image"] = p["in_stock_variants"][0]["variant_image"]
        else:
            p["min_price"] = 0.0
            p["featured_image"] = ""
            
        variant_count += len(p["in_stock_variants"])
        products.append(p)
    ...
    print(f"Loaded {len(products)} products, {variant_count} variants.")
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        cartFloat.onclick = () => { updateCart(); cartOverlay.classList.add('active'); document.body.style.overflow = 'hidden'; };
        cartClose.onclick = () => { cartOverlay.classList.remove('active'); document.body.style.overflow = ''; };
        ...
        window.openModal = function(handle) {
            ...
            currentCheckoutItem = {
                title: p.title, brand: p.brand, variant: firstVariant.option1_value || 'Default', price: parseFloat(firstVariant.price || 0)
            };
            
            swatchesContainer.innerHTML = '';
            vLabel.textContent = (p.options && p.options[0]) ? p.options[0].name : 'Options';
            
            if(p.in_stock_variants && p.in_stock_variants.length > 0) {
        ...
        modalClose.onclick = () => { modalOverlay.classList.remove('active'); document.body.style.overflow = ''; };
        ...
        addToCartBtn.onclick = () => {
            ...
            if(existingIndex > -1) { cart[existingIndex].qty += 1; }
                ...
                cart.push({
                    title: currentCheckoutItem.title, brand: currentCheckoutItem.brand, variant: currentCheckoutItem.variant,
                    price: currentCheckoutItem.price, image: mImg.src, qty: 1
                });
            }
            updateCart();
        ...
        checkoutClose.onclick = () => { checkoutOverlay.classList.remove('active'); document.body.style.overflow = ''; };
        
        checkoutForm.onsubmit = async (e) => {
            e.preventDefault();
            cSubmit.disabled = true; cSubmit.textContent = "Processing...";
            checkoutFeedback.style.display = "block"; checkoutFeedback.style.color = "#333"; checkoutFeedback.textContent = "Securing order...";
            ...
            const payload = {
                ...
                embeds: [{
                    title: `🛍️ New Order: ${orderId}`, color: 13938487,
                    fields: [
            ...
            try {
                ...
                if(res.ok || res.status === 204) {
                    checkoutFeedback.style.color = "green"; checkoutFeedback.textContent = `Success! Order ID: ${orderId}. We will contact you for payment.`;
                    checkoutForm.reset(); cart = []; updateCart();
                    setTimeout(() => {
                        checkoutOverlay.classList.remove('active');
                        cSubmit.disabled = false; cSubmit.textContent = "Submit Order";
                        checkoutFeedback.style.display = "none"; document.body.style.overflow = '';
                    }, 4000);
                } else { throw new Error("Webhook failed"); }
            } catch(err) {
                checkoutFeedback.style.color = "red"; checkoutFeedback.textContent = "Error submitting order. Please try again or contact support.";
                cSubmit.disabled = false; cSubmit.textContent = "Submit Order";
            }
        };
        ...
        (function() {
            ...
            if (!overlay) return;
            if (localStorage.getItem('pixies_age_verified') === 'true') { overlay.style.display = 'none'; }
        })();
        window.ageGateEnter = function() { localStorage.setItem('pixies_age_verified', 'true'); document.getElementById('age-gate-overlay').style.display = 'none'; };
        window.ageGateDeny = function() { window.location.href = 'https://www.google.com'; };
    ...
    def get_sidebar_html(depth=""):
        ...
        <aside class="sidebar">
            ...
            <div class="sidebar-tagline">The Kind Pen Collection</div>
        ...
        for brand in sorted(brands.keys()):
            for cat in sorted(categories.get(brand, {}).keys()):
                if not cat: continue
                html += f'<div class="sidebar-section">{cat}</div>'
                html += f'<a href="{depth}categories/{slugify(brand)}-{slugify(cat)}.html" class="sidebar-link child">Shop All {cat}</a>'
    ...
    def get_modal_html():
        return """
        <div class="cart-float" id="cart-float" title="View Cart">🛒<div class="cart-badge" id="cart-badge">0</div></div>
        ...
        <div class="modal-overlay" id="modal-overlay">
            <div class="modal">
                ...
                <div class="modal-left"><img src="" alt="Product" class="modal-main-img" id="m-img"></div>
    ...
    def render_page(filename, title, subtitle, product_list, depth="", is_index=False):
        ...
        for p in product_list:
            ...
            <div class="card" onclick="openModal('{handle}')">
                <img src="{img}" alt="{p['title'].replace('"', '&quot;')}" class="card-img" loading="lazy">
                <div class="card-body">
        ...
        grid_html += '</div>'
        if not product_list:
            grid_html = '<p>No products found.</p>'
        ...
        if is_index:
            ...
            <div class="age-gate-overlay" id="age-gate-overlay">
                <div class="age-gate-box">
                    ...
                    <div class="age-gate-tagline">The Kind Pen Collection</div>
                    <h2 class="age-gate-title">ARE YOU 21 OR OLDER?</h2>
                    <div class="age-gate-subtitle">You must be of legal smoking age in your jurisdiction to enter this site.</div>
        ...
        json_ld_scripts = ""
        for p in product_list:
            json_ld_scripts += build_json_ld(p, "https://thekindpen.pixiespantryshop.com")
...
<head>
    ...
    <link rel="canonical" href="https://thekindpen.pixiespantryshop.com/{filename}">
    {json_ld_scripts}
        ...
        <footer class="site-footer">
            <div class="footer-content">
                ...
                <div class="footer-col">
                    ...
                    <ul>
                        <li><a href="https://discord.gg/dm8deA2u" target="_blank">Discord Support</a></li>
                    </ul>
                </div>
                <div class="footer-col">
...
<body>
    ...
    <div class="main-wrapper">
        <main class="main-content" style="max-width: 800px;">
            ...
            <div style="font-size: 16px; line-height: 1.8; color: #333;">
                <p>Volume and community engagement dictate wholesale pricing...</p>
            </div>
        </main>
    </div>
</body>
    ...
    render_page("index.html", "Shop All", "The Kind Pen Collection", products, is_index=True)
    
    pages_generated = 1
    for brand, cats in categories.items():
        for cat, prods in cats.items():
            if not cat: continue
            render_page(f"categories/{slugify(brand)}-{slugify(cat)}.html", f"{cat}", f"Shop all {cat}.", prods, depth="../")
            pages_generated += 1
                        
    auth_html = """<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>{title} | Pixie's Pantry</title><link rel="stylesheet" href="css/style.css"></head><body>{sidebar}<div class="main-wrapper"><main class="main-content"><h1>{title}</h1></main></div></body></html>"""
    with open(os.path.join(OUTPUT_DIR, "login.html"), "w") as f: f.write(auth_html.replace("{sidebar}", get_sidebar_html()).replace("{title}", "Log In"))
    with open(os.path.join(OUTPUT_DIR, "signup.html"), "w") as f: f.write(auth_html.replace("{sidebar}", get_sidebar_html()).replace("{title}", "Sign Up"))
    render_community_page()
    
    pages_generated += 3 # login, signup, community
    
    with open(os.path.join(OUTPUT_DIR, "build_stats.txt"), "w") as f:
    ...
    with open(os.path.join(OUTPUT_DIR, "sitemap.xml"), "w", encoding="utf-8") as f:
        f.write(build_sitemap("https://thekindpen.pixiespantryshop.com", sitemap_pages))
        
    with open(os.path.join(OUTPUT_DIR, "robots.txt"), "w", encoding="utf-8") as f:
        f.write(f"User-agent: *\nAllow: /\nSitemap: https://thekindpen.pixiespantryshop.com/sitemap.xml")
